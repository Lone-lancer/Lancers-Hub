...
const WebcamButtonWrapper = styled.div`
  margin-top: 20px;
  text-align: center;
...
export default function LoginDialog() {
  ...
  // Extract the fullName from the URL and derive the first name
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const fullName = params.get('fullName') || '';
    const firstName = fullName.split('%20')[0] || 'Guest'; // Default to "Guest" if no name is provided
    setName(firstName);
  }, []);

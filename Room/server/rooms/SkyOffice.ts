...
export class SkyOffice extends Room<OfficeState> {
  ...
  async onCreate(options: IRoomData) {
    const { name, description, password, autoDispose, roomId } = options;
    this.name = name;
    this.description = description;
    this.autoDispose = autoDispose;

    // Apply the custom roomId if provided
    if (roomId) {
      this.roomId = roomId;
      console.log(`Room created with custom ID: ${this.roomId}`);
    } else {
      console.log(`Room created with default ID: ${this.roomId}`);
    }
